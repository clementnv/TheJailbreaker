...
if (state != "dead") {
	if (press_right) {
		...
		} else if (!place_meeting(x+walkspd*3, y, o_Wall) && !press_up && !press_down) {
			x += walkspd;
			state = "walk";
		} else if (!place_meeting(x+walkspd*3, y-walkspd*3, o_Wall) && press_up && !press_down) {
			x += walkspd/2;
			y -= walkspd/2;
			state = "walk";
		} else if (!place_meeting(x+walkspd*3, y+walkspd*3, o_Wall) && !press_up && press_down) {
	...
	if (press_up && !press_left && !press_right) {
		image_angle = 0;
	...
	if (press_down && !press_left && !press_right) {
		image_angle = 180;
		if (place_meeting(x, y+walkspd, o_Key)) {
			key = true;
			instance_destroy(o_Key);
