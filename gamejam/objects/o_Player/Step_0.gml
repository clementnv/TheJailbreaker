...
if (state != "dead") {
	...
	if (place_meeting(x, y, o_Water)) {
		...
		if (!audio_is_playing(eau_bruitage)) {
			audio_play_sound(eau_bruitage, 10, false)
		}
	} else {
		walkspd = 7;
	}
	...
	if (place_meeting(x, y, o_Mattraque) && !o_Mattraque.player) {
		mattraque = true;
		o_Mattraque.player = true;
		audio_play_sound(coup_vide_bruitage, 10, false);
	} 
...
if (state == "idle") {
	...
	if(gun) {
		...
		sprite_index = s_Prisonnier_Walk_Mattraque;
	} else {
	...
	if(gun) {
		...
		if (!audio_is_playing(matraque_bruitage)) {
			audio_play_sound(matraque_bruitage, 10, false)
		}
	} else {
